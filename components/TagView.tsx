...
export default async function TagView({ tagType }: {  tagType: TagTypeEnum }) {
    ...
    return (
        <div className='flex flex-col flex-1 overflow-x-auto w-full gap-4 bg-base-100 shadow-sm max-sm:p-4 sm:p-6 max-md:w-full md:w-3xl'>
        {
            tags.map((tag: TagVO) => (
                <div key={tag.id} className='flex justify-between items-baseline min-w-2xs'>
                    <Link
                        className='link text-base-content/80 transition-colors hover:text-primary text-sm'
